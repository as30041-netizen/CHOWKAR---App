-- ============================================================================
-- RPC: KEYWORD_AWARE_FEED
-- Updates get_home_feed to match user skills against Title/Description
-- ============================================================================

-- DROP first to allow return type change (adding is_recommended)
...
BEGIN
    ...
    -- SANITIZATION: Escape special regex characters to prevent crashes with "C++" or similar
    ...
    SELECT 
        ...
        j.bid_count,
        -- REFINED LOGIC: 
        -- 1. Exact Category Match OR
        -- 2. Keyword Match (Regex) in Title/Desc (Case Insensitive)
        (
    ...
    ORDER BY 
        -- BOOSTING: Recommended jobs first
        CASE WHEN (
            v_user_skills IS NOT NULL AND (
                j.category = ANY(v_user_skills) OR 
                (v_skill_pattern IS NOT NULL AND v_skill_pattern <> '' AND (j.title ~* v_skill_pattern OR j.description ~* v_skill_pattern))
