-- ============================================================================
-- FIX: PRIORITIZE ACTIVE JOBS (Fixes "Disappearing Active Jobs")
-- ============================================================================

BEGIN;

-- 1. DROP old version first to avoid argument mismatches
DROP FUNCTION IF EXISTS get_my_jobs_feed(UUID, INTEGER, INTEGER);

-- 2. RECREATE with improved sorting logic
...
BEGIN
  ...
  ORDER BY 
    CASE WHEN UPPER(j.status::TEXT) IN ('OPEN', 'IN_PROGRESS') THEN 0 ELSE 1 END ASC, -- Active first!
