-- CRITICAL_FIX_BIDS.sql
-- Purpose: Fix "operator does not exist: timestamp with time zone >= numeric"
-- Also ensures RPC visibility by setting search_path.

BEGIN;

-- 1. FIX: Policy Function (Compare TIMESTAMPTZ directly)
...
BEGIN
    ...
    IF p_action = 'POST_JOB' THEN
        ...
        AND created_at >= v_period_start; -- FIX: Direct comparison
...
-- 2. FIX: Bid Placement RPC (Ensure visibility)
...
BEGIN
    ...
    -- Insert Bid (This will fire the limit trigger)
    ...
    VALUES (v_poster_id, 'New Bid Received', 'Someone bid ₹' || p_amount || ' on ' || v_job_title, 'INFO', p_job_id, extract(epoch from now())::bigint);
