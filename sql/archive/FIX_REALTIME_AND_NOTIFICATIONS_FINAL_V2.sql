...
-- Use DO blocks for logic to handle existing objects gracefully
DO $$
BEGIN
...
-- Add tables to the realtime publication (using a DO block to avoid errors if already present)
DO $$
BEGIN
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE jobs;
    EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'Table jobs already in publication';
    END;
    
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE bids;
    EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'Table bids already in publication';
    END;
    
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE notifications;
    EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'Table notifications already in publication';
    END;
    
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE chat_messages;
    EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'Table chat_messages already in publication';
    END;
END $$;
