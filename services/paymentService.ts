...
export const initiatePayment = async (packId: string, userId: string, userEmail?: string, userPhone?: string): Promise<PaymentResult> => {
    ...
    try {
        ...
        if (!orderData || orderData.error) {
            logger.error('[Payment] Order Data Error:', orderData?.error);
        ...
        return new Promise((resolve) => {
            const options: RazorpayOptions = {
                ...
                handler: async (response: any) => {
                    ...
                    try {
                        console.log('[Payment] Verifying directly via Edge Function...');
            ...
            rzp1.on('payment.failed', function (response: any) {
                logger.error('[Payment] Failure:', response.error);
...
export const initiatePremiumPayment = async (userId: string, userEmail?: string, userPhone?: string): Promise<PaymentResult> => {
    ...
    try {
        ...
        logger.error('[Payment] Premium Exception:', err);
