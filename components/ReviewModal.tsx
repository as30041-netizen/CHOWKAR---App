...
export const ReviewModal: React.FC<ReviewModalProps> = ({ isOpen, onClose, onSubmit, revieweeName }) => {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 animate-fade-in pointer-events-none">
            ...
            {/* Main Container */}
            <div className="bg-white dark:bg-gray-950 rounded-[2rem] w-full max-w-md p-6 sm:p-8 shadow-[0_32px_128px_-16px_rgba(0,0,0,0.4)] animate-slide-up relative pointer-events-auto border border-gray-100 dark:border-gray-800 transition-all max-h-[85vh] overflow-y-auto custom-scrollbar flex flex-col">

                <div className="flex items-center gap-4 mb-6 border-b border-gray-100 dark:border-gray-800 pb-6">
                    <button
                        onClick={onClose}
                        className="p-3.5 rounded-2xl bg-gray-50 dark:bg-gray-900 text-gray-400 hover:text-gray-900 dark:hover:text-white transition-all active:scale-95 border border-transparent hover:border-gray-200 dark:hover:border-gray-700 group min-w-[44px] min-h-[44px] flex items-center justify-center"
                    >
                        <ArrowLeft size={20} strokeWidth={2.5} className="group-hover:-translate-x-0.5 transition-transform" />
                    </button>
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-600 flex items-center justify-center text-white shadow-lg shadow-amber-500/20 rotate-3 group shrink-0">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <div className="p-0.5 px-2 bg-emerald-50 dark:bg-emerald-950 rounded-full border border-emerald-100 dark:border-emerald-800 flex items-center gap-1.5 font-bold text-[9px] uppercase tracking-widest text-emerald-600 dark:text-emerald-400">
                                    <ShieldCheck size={10} strokeWidth={3} /> Verified
                                </div>
                                <span className="text-sm font-semibold text-gray-400 line-clamp-1">{revieweeName}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="text-center mb-8 relative">
                    ...
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-amber-500 mt-4 min-h-[1.5em] animate-pulse">
                        {language === 'en' ? getMoodText().en : getMoodText().hi}
                    </p>
                </div>

                <div className="mb-8">
                    <p className="text-center text-[10px] font-black uppercase tracking-widest text-gray-400 mb-4">
                ...
                <div className="mt-6 flex items-center justify-center gap-2 text-[9px] font-black uppercase tracking-widest text-gray-300/60 dark:text-gray-700 animate-fade-in">
