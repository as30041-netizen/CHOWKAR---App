...
export const WalletView: React.FC<WalletViewProps> = ({ onShowBidHistory }) => {
  ...
  const handleAddMoney = () => {
    setUser(p => ({...p, walletBalance: p.walletBalance + 100}));
  ...
  return (
    <div className="p-4 animate-fade-in">
        <div className="bg-emerald-600 rounded-2xl p-6 text-white mb-6 shadow-lg relative overflow-hidden animate-pop">
            ...
            <button onClick={handleAddMoney} className="bg-white text-emerald-700 px-4 py-2 rounded-lg text-sm font-bold shadow-sm">+ {t.addMoney}</button>
        </div>
